#!/usr/bin/perl
# Accurate distance curve from fragment database (all 4 fragment end combinations)
# 2015/07/29 Data for distance normalization
# 2015/07/09 Distance curve calculation
...
# Chromosome length - distance = number of possible pairs at each distance
# Sum for each distance divided by total reads gives the contact probability distance curve

if(@ARGV != 6 or $ARGV[0] eq '--help'){
	die "Usage : $0 -i [fragment.db] -o [output file] -l [file of chromosome length (fai or chr<TAB>length)]\n";
}
...
foreach my $chr(@chrs){
	...
	while(my $ref = $sth_intra->fetchrow_arrayref()){
		...
		# Double score for distance below 10kb
